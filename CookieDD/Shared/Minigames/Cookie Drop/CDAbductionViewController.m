//
//  CDAbductionViewController.m
//  cowTest
//
//  Created by Gary Johnston on 6/15/14.
...
@interface CDAbductionViewController() <CDMiniGamePopupViewDelegate, CDMainButtonViewControllerDelegate, CDStandardPopupViewDelegate, CDAccountPopupViewDelegate, CDAwardPopupViewControllerDelegate>

@property (strong, nonatomic) CDAbductionScene *scene;
...
@implementation CDAbductionViewController
...
//    skView.showsNodeCount = YES;
    ...
    SKScene * scene = [CDAbductionScene sceneWithSize:CGSizeMake(skView.frame.size.height, skView.frame.size.width)];
...
- (void)viewWillLayoutSubviews
{
    ...
    {
        SKView *skView = (SKView *)self.view;
        self.scene = [CDAbductionScene sceneWithSize:skView.bounds.size];
...
- (void)openShop
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
    {
        [self dismissViewControllerAnimated:NO completion:^{
            if ([self.delegate respondsToSelector:@selector(storeButtonHitInAbductionMinigame)])
            {
                [self.delegate storeButtonHitInAbductionMinigame];
            }
...
- (void)abductionMinigameWillPresentDifficultyScreen:(CDAbductionScene *)scene
{
    ...
    NSInteger highScore = [highScoreDefault integerForKey:CowAbductionHighScoreDefault];
    ...
    _difficultyView.gameNameLabel.text = @"Cow Abduction";
}
...
- (void)abductionMinigameSceneDidEnd:(CDAbductionScene *)scene WithScore:(int)score WithGoalScore:(int)goalScore WithWin:(BOOL)didWin
{
    ...
    if (!self.didCreatePopup)
    {
        ...
        if (score > highScore)
        {
            [highScoreDefault setInteger:score forKey:CowAbductionHighScoreDefault];
        ...
        _difficultyView.minigameName = @"Cow Abduction";
        ...
        if (didWin)
        {
            ...
            if (scene.difficulty == gameDifficultyLevelHard)
            {
                ...
                BOOL doNotAwardGem = [userDefault boolForKey:CowAbductionAwardGemDefault];
                
                if (!doNotAwardGem)
                {
                    awardPopup.awardGem = YES;
                    [userDefault setBool:YES forKey:CowAbductionAwardGemDefault];
                }
                else
                {
...
-(void)helpButtonWasHitOnMiniGamePopupView:(BOOL)isOpen
{
    if (isOpen)
    {
        ...
        _helpPopup.whatAmILoading = @"cowAbductionHelp";
...
- (void)difficultyWasSelectedOnMiniGamePopupView:(CDMiniGamePopupView *)miniGamePopupView WithDifficulty:(int)difficulty
{
    ...
    _scene.difficulty = difficulty;
    [_scene startMiniGame];
}
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void)
                   {
                       [self dismissViewControllerAnimated:NO completion:^{
                           [_loadingScreen.view removeFromSuperview];
                           if ([self.delegate respondsToSelector:@selector(replayButtonWasHitInAbductionMinigame)])
                           {
                               [self.delegate replayButtonWasHitInAbductionMinigame];
                           }
                       }];
                   });
}
...
- (void)mainButtonSubButtonWasHitWithIndex:(int)buttonIndex
{
    ...
    {
        DebugLog(@"BackButton - CDAbductionViewController");
        _mainButtonViewController.view.clipsToBounds = YES;
        [self loadingScreenHandler];
